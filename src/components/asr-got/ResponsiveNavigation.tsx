...
import { Menu, X, ChevronDown, ChevronRight, Brain, TreePine, Network, BarChart3, Settings, Code, Download, Database, History, Microscope } from 'lucide-react';
      ...
      {navigationItems.map(item => <div key={item.id}>
        ...
        if (item.children && item.children.length > 0) {
          ...
          setIsMobileMenuOpen(false);
            ...
            {item.children && item.children.length > 0 && (expandedItems.has(item.id) ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />)}
          </Button>
          ...
          setIsMobileMenuOpen(false);
                  ...
                  {child.badge && <Badge variant="outline" className="ml-auto text-xs">
                      {child.badge}
                    </Badge>}
                </Button>)}
            </div>}
