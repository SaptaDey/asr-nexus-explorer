        ...
        <TabsContent value="progress" className="space-y-6">
          <Card className="card-gradient">
            ...
            <CardContent className="space-y-6">
              ...
              {researchContext?.topic && <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-4 rounded-lg border">
                  ...
                  {researchContext.field && <p className="text-sm"><strong>Field:</strong> {researchContext.field}</p>}
                </div>}
              ...
              <div>
                ...
                <Progress value={(currentStage + 1) / 9 * 100} className="h-3 bg-purple-100" />
              </div>
              ...
              <div className="space-y-4">
                ...
                <ScrollArea className="h-96 w-full">
                  <div className="space-y-4 pr-4">
                    ...
                    return <Card key={index} className={`card-gradient ${isCurrentStage ? 'border-purple-200 border-2' : ''}`}>
                          <CardHeader className="pb-2">
                            <div className="flex items-center gap-2">
                              {isCompleted ? <CheckCircle className="h-4 w-4 text-green-500" /> : isCurrentStage ? <Loader2 className="h-4 w-4 animate-spin text-purple-500" /> : <div className="h-4 w-4 rounded-full border-2 border-gray-300" />}
                          ...
                          {isCompleted && <CardContent className="pt-0">
                              <div className="text-sm prose prose-sm max-w-none">
                                <ReactMarkdown remarkPlugins={[remarkGfm]} components={{
                            h1: ({
                              children
                            }) => <h1 className="gradient-text text-lg font-bold mb-2">{children}</h1>,
                            h2: ({
                              children
                            }) => <h2 className="gradient-text text-base font-semibold mb-2">{children}</h2>,
                            h3: ({
                              children
                            }) => <h3 className="text-purple-700 text-sm font-semibold mb-1">{children}</h3>,
                            p: ({
                              children
                            ...
                            code: ({
                              children
                            }) => <code className="bg-purple-100 px-1 py-0.5 rounded text-xs">{children}</code>,
                            pre: ({
                              children
                            }) => <pre className="bg-gray-100 p-2 rounded text-xs overflow-x-auto">{children}</pre>,
                            table: ({
                              children
                            }) => <table className="border-collapse border border-gray-300 text-xs w-full mb-2">{children}</table>,
                            th: ({
                              children
                            }) => <th className="border border-gray-300 px-2 py-1 bg-purple-50 font-semibold">{children}</th>,
                            td: ({
                              children
                          ...
                          {isCurrentStage && !isCompleted && <CardContent className="pt-0">
              ...
              {!isProcessing && currentStage < 9 && processingMode === 'manual' && <div className="space-y-2">
                  <Button onClick={handleContinueToNext} className="w-full gradient-bg" disabled={currentStage >= 9}>
                    <Zap className="h-4 w-4 mr-2" />
              ...
              {processingMode === 'automatic' && currentStage < 9 && <div className="bg-gradient-to-r from-blue-50 to-purple-50 p-4 rounded-lg border border-blue-200">
              ...
              {currentStage >= 4 && graphData.nodes.some(n => n.type === 'evidence') && <div className="space-y-4">
                  <h3 className="font-semibold gradient-text flex items-center gap-2">
                    <BarChart3 className="h-4 w-4" />
                    Enhanced Visual Analytics & Charts
                    {analyticsGenerating && <div className="animate-spin text-primary ml-2">
                        <Loader2 className="h-4 w-4" />
                      </div>}
                  </h3>
                  
                  {analyticsFigures.length > 0 && <div className="bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-200 rounded-lg p-4">
                      <p className="text-sm text-blue-700 font-medium mb-2">
                        📊 {analyticsFigures.length} Publication-Ready Charts Generated
                      </p>
                      <div className="grid grid-cols-2 lg:grid-cols-3 gap-2 text-xs">
                        {analyticsFigures.slice(0, 6).map((fig, idx) => <span key={idx} className="bg-white/80 px-2 py-1 rounded border">
                            {fig.title.substring(0, 30)}...
                          </span>)}
                      </div>
                    </div>}
                  
                  <VisualAnalytics graphData={graphData} currentStage={currentStage} geminiApiKey={apiKeys.gemini} researchContext={{
                topic: researchContext?.topic || '',
                field: researchContext?.field || '',
                objectives: researchContext?.objectives || [],
                hypotheses: researchContext?.hypotheses || [],
              ...
              {stageResults.length >= 9 && <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-lg border border-green-200">
                  <div className="flex items-center gap-2 mb-2">
